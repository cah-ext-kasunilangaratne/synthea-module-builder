...
class LoadModule extends Component {
  ...
  fetchMongoList(){
    ...
    fetch(`http://`+process.env.REACT_APP_BACKEND+`:5000/module`, get_options)
      .then(response => response.json())
        .then(data => {
  ...
  fetchMongoVersionsList(name){
    ...
    fetch(`http://`+ process.env.REACT_APP_BACKEND +`:5000/module/?name=`+name, get_options)
      .then(response => response.json())
        .then(data => {
          this.setState({
            mongoVersions: 
  ...
  fetchMongoModule(id){
    ...
    fetch(`http://`+process.env.REACT_APP_BACKEND+`:5000/module/`+ id, get_options)
      .then(response => response.text())
      .then(data => this.loadModule(data))
      .then(this.setState({
        Modules: null
  ...
  setActive(branch){
    ...
    fetch(`http://`+process.env.REACT_APP_BACKEND+`:5000/module/` + id, get_options)
        ...
        .then(data => { 
            ...
            fetch('http://'+process.env.REACT_APP_BACKEND+':5000/module/' + id, put_options)
  ...
  setInactive(branch){
    ...
    fetch(`http://`+process.env.REACT_APP_BACKEND+`:5000/module/?name=`+branch.name, get_options)
        .then(response => response.json())
        .then(data => {
            data.forEach(function(current_value, index, data){
              if (data[index].active && data[index]._id != id) { 
                fetch(`http://`+process.env.REACT_APP_BACKEND+`:5000/module/`+data[index]._id, get_options)
                ...
                .then(module_data => {
                  ...
                  fetch('http://'+process.env.REACT_APP_BACKEND+':5000/module/' + module_data._id, put_options)
                  .then(get_response => get_response.json())
                  .catch(function(error) {
                    console.log(error)
                  })
